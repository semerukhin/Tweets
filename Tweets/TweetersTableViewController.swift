...
class TweetersTableViewController: FetchedResultsTableViewController {
   ...
   private func updateUI() {
      if let context = container?.viewContext, mention != nil {
         ...
         request.predicate = NSPredicate(format: "any tweets.text contains[c] %@ and !handle beginswith[c] %@", mention!, "MEC30")
...
extension TweetersTableViewController { // MyTableViewControllerUsesNSFetchedResultsContoller
